  ...
  return (
    <main className="min-h-screen bg-[#1C164B] text-white flex flex-col justify-between overflow-x-clip">
      ...
      <section className="relative pt-28 sm:pt-48 pb-16 sm:pb-24 px-5 sm:px-12 mx-auto max-w-7xl w-full border-b border-[#8A5FA8]/30">
        ...
        {/* Narrative Columns */}
        <div className="mt-12 sm:mt-16 pt-8 sm:pt-12 border-t border-[#8A5FA8]/30 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14">
          <div className="lg:col-span-6 space-y-4 text-sm sm:text-base text-[#EAE7DC]/80 font-light leading-relaxed">
            <p>
        ...
        {/* Statistics Strip */}
        <div className="mt-14 sm:mt-20 pt-8 sm:pt-10 border-t border-[#8A5FA8]/30">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
            {STATS.map((stat) => (
              <div key={stat.label} className="space-y-1 sm:space-y-1.5">
                ...
                <p className="font-display text-xs sm:text-sm text-[#98DAF6] font-medium leading-snug">
                  {stat.label}
                </p>
              </div>
            ))}
      ...
      <section className="py-16 sm:py-28 px-5 sm:px-12 mx-auto max-w-7xl w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-8 pt-2">
              {CAPABILITIES.map((cap, i) => (
                <div
                  key={i}
                  className="flex items-baseline gap-3 py-2.5 border-b border-[#8A5FA8]/20 text-xs sm:text-sm font-light text-white/90"
                >
                  <span className="font-display text-xs text-[#98DAF6] font-bold">
                    0{i + 1}
                  </span>
                  <span>{cap}</span>
                </div>
          ...
          <div className="lg:col-span-5 space-y-5 rounded-2xl bg-[#1C164B]/60 border border-[#8A5FA8]/30 p-6 sm:p-8">
            <h3 className="font-display text-xl sm:text-2xl font-bold text-white">
              Dubai, United Arab Emirates
            </h3>
            <p className="text-sm text-[#EAE7DC]/75 leading-relaxed font-light">
            ...
            <div className="pt-2 flex flex-wrap gap-3">
              ...
              <Link
                href="/"
                className="inline-flex items-center justify-center px-4 py-2.5 text-xs sm:text-sm font-semibold text-[#98DAF6] hover:text-white transition-colors"
              >
                Return to Homepage →
              </Link>
            </div>
